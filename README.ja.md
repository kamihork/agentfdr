...
agentfdr board              # 稼働中の全セッションのライブカンバン。tmux セッションには指示も送れる
...
メインビューはタブで切替: **タイムライン / テーブル表示 / プロンプト / サブエージェント / 使用量 / 比較 / 検索**(サブエージェントはそれが起動されたセッションでのみ表示)。プロンプトの行・異常チップ・サブエージェントの起点ターン・検索結果をクリックすると、タイムラインの該当ターンへ自動でジャンプします。ヘッダーの絞り込みボックスで、全プロジェクト横断でセッション一覧を絞り込めます。凡例のツール色をクリックするとツールレーンを絞り込み。**レポートをコピー** で blame の Markdown をクリップボードへ、**📤 カード** でセッションのサマリ画像(PNG)を書き出し、**● LIVE** で実行中セッションを自動更新(`agentfdr watch` なら最初からオン)。言語・テーマはヘッダーで切替でき、表示状態は URL で共有できます(`?theme=dark&tab=usage&sel=95`)。
...
- **指示** — tmux 上で動くセッション(`tmux new-session … claude` 系のランチャーなら何でも)のカードには *指示…* / *承認* / *拒否* / *停止* が付きます。テキストはキーボードで打つのと同じようにペインに入力・送信され(複数行は1回のペーストとして)、承認/拒否は開いているダイアログに Enter/Esc を、停止はターンの中断を送ります。承認/拒否/停止は2クリック確認で、モーダルは出しません。Desktop アプリとバックグラウンド(`--bg`)のセッションには届くペインがないため閲覧のみです。
...
指示のエンドポイントはボードページ自身からの same-origin な `POST` にしか応答せず、サーバーは `127.0.0.1` に留まります — エージェントに入力できるページが他のタブから届いてはいけません。`agentfdr board --json` は同じボードを1回だけ出力します(スクリプト用)。
