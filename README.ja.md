...
- 🚨 **異常検知** — ツールループ、エラー連続、コンテキスト肥大、トークン急増、キャッシュスラッシング、同一ファイル連続編集、意図ドリフト、refusal を自動で検知してフラグ付け
- ⧉ **サブエージェントツリー** — サブエージェントに委譲した作業は別トランスクリプトに記録され、セッション合計には一切現れません。**サブエージェント**タブが各エージェントを起動元のターンに繋ぎ直し(入れ子も含む)、ターン数・ツール呼び出し・課金トークン・異常を一覧します
- 📡 **ライブウォッチ** — `agentfdr watch` で実行中セッションを自動追尾
- 🗂 **ライブセッションボード** — `agentfdr board` で稼働中の全 Claude Code セッションを1枚のカンバンに: **要対応**(許可プロンプト/質問)・**作業中**・**待機中**。今の依頼、エージェントが今まさにしていること、ここまでのコスト、そして2つのセッションが同じリポジトリ/ファイルを編集している時の警告。CLI と Desktop アプリのセッションを並べて表示し、tmux 上のセッションには画面から指示・承認・拒否・中断を送れる
...
- 📤 **共有用セッションカード** — ワンクリックでPNGのサマリカード(コスト・トークン・キャッシュ効率・異常・コンテキスト曲線)をX向けサイズで書き出し
...
agentfdr board              # 稼働中の全セッションのライブカンバン。tmux セッションへの指示・新規起動
agentfdr statusline-tap     # statusline パススルー。5h/7d のプラン使用率をアカウント別に記録(ボード用)
...
メインビューはタブで切替: **タイムライン / テーブル表示 / プロンプト / サブエージェント / 使用量 / 比較 / 検索**(サブエージェントはそれが起動されたセッションでのみ表示)。プロンプトの行・異常チップ・サブエージェントの起点ターン・検索結果をクリックすると、タイムラインの該当ターンへ自動でジャンプします。ヘッダーの絞り込みボックスで、全プロジェクト横断でセッション一覧を絞り込めます。凡例のツール色をクリックするとツールレーンを絞り込み。**レポートをコピー** で blame の Markdown をクリップボードへ、**📤 カード** でセッションのサマリ画像(PNG)を書き出し、**● LIVE** で実行中セッションを自動更新(`agentfdr watch` なら最初からオン)、**🗂 ボード** でライブセッションボードへ(ボードの各カードからもここへ戻れます)。タイムラインの見出しとタブタイトルにはセッションの年月日が入ります。言語・テーマはヘッダーで切替でき、表示状態は URL で共有できます(`?theme=dark&tab=usage&sel=95`)。
...
- **状態別の列** — 🔴 *要対応*(許可プロンプト、質問、プラン承認)、🟢 *作業中*、🟡 *待機中*(完了して次の依頼待ち)、加えて *最近終了*。ワンクリックでプロジェクト別・アカウント別にも切り替え。
- **手動レーン** — 表示を *手動レーン* に切り替えると、ボードを自分流に仕分けできます: 自分で名前を付けた列(初期値: 今やる / 後で / 監視中 / 保留)にカードをドラッグして振り分け・並べ替え、レーン名はその場で変更・削除。新しいセッションは *未分類* に入ります。レーンと配置はブラウザに保存され、セッション ID をキーにするので再起動したセッションもレーンに留まります。状態チップはそのままセッションが実際に何をしているかを示します。
- **各カード** — あなたが入力した依頼、エージェントが今この瞬間にしていること(または最後の発言)、その依頼以降のターン数 / ツール呼び出し / エラー / コンテキストサイズ / 概算コスト、編集したファイル、現在のタスクに立った異常フラグ。
- **2アカウントを1画面で** — セッションに **CLI** / **Desktop**(Claude デスクトップアプリ内の Claude Code)のバッジが付き、ヘッダーには各アプリがどのアカウントでログインしているかが出て、フィルタでどちらかに絞れます。Desktop セッションはアプリに表示されるタイトル付きで、アプリの一覧にはあるがプロセスが*休止*中(次のメッセージまで動かない)のセッションも *休止* 印付きの待機中カードとして並ぶので、ボードとアプリのサイドバーで「開いているもの」が一致します。CLI と別アカウントの Desktop セッションには *別アカウント* 印が付きます。
- **セッション間の影響** — 同じリポジトリ上のセッション(worktree は親リポジトリに解決)をまとめ、複数のライブセッションが編集しているファイルをバナーで名指しします。
- **指示** — tmux 上で動くセッション(`tmux new-session … claude` 系のランチャーなら何でも)のカードには *指示…* / *承認* / *拒否* / *停止* が付きます。テキストはキーボードで打つのと同じようにペインに入力・送信され(複数行は1回のペーストとして)、承認/拒否は許可プロンプトに Enter/Esc を送ります(質問やプラン承認など他のダイアログは既定値が異なるので、代わりに *Enter* / *Esc* と明示したボタンになります)。停止はターンの中断。承認/拒否/停止は2クリック確認で、モーダルは出しません。カードを描いた後にセッションの状態が変わっていればサーバーがキー送信を拒否するので、あるダイアログ向けのクリックが次のダイアログに届くことはありません。セッション内から起動した `claude -p` / SDK プロセスはペインを共有するため閲覧のみで表示します。Desktop アプリとバックグラウンド(`--bg`)のセッションには届くペインがないため閲覧のみです。
- **セッションの起動** — *+ 新規セッション* でランチャーが開きます: ディレクトリ(候補は過去に Claude Code を動かした場所)、任意の最初の依頼、モデル、そのディレクトリの前回の会話を続けるかどうか。ボードは `claude-<dir>-<cksum>` という名前の切り離し tmux セッションを作ります — パスをキーにした `tmux new-session … claude` 系のシェルラッパーと同じ命名なので `cc` のようなランチャーはそのまま接続できます — そこで `claude` を起動し、登録され次第カードが現れます(同じディレクトリの2本目は `-2` が付きます)。この方法で起動したセッションは他の tmux セッションと同様に指示できます。
- **アカウントごとのプラン上限** — Claude Code が statusline に出している 5時間 / 7日 の使用率バーを、アカウントごとにボード上部へ。この数値はディスク上のどこにも残らないため、agentfdr は記録するための statusline パススルーを同梱しています: `~/.claude/settings.json` の statusline コマンドの前に `agentfdr statusline-tap |` を付けるだけ(自前の statusline が無ければ `agentfdr statusline-tap --line` 単体で `[model] dir · 5h 26% · 7d 9%` の1行を出します)。読み取り値は `~/.agentfdr/rate-limits.json` にアカウント別・リセット時刻付きで保存され、別の `CLAUDE_CONFIG_DIR` で動く2つ目のアカウントは自分のキーに記録します。tap を入れるまでは、その旨を1行出すだけで邪魔をしません。

  ```json
  "statusLine": { "type": "command", "command": "agentfdr statusline-tap | ~/.claude/statusline-command.sh" }
  ```
- **軽い設計** — 稼働中のトランスクリプトは 100 MB を超えることがあるので、ボードは各ファイルの末尾 2 MB だけを読みます(`AGENTFDR_BOARD_TAIL_MB` で変更可)。1回のポーリングは数ミリ秒。`≥` 付きの数値は、現在のタスクがその範囲より前に始まったことを意味します。

指示と起動のエンドポイントはボードページ自身からの same-origin な `POST` にしか応答せず、サーバーは `127.0.0.1` に留まります — エージェントに入力できるページが他のタブから届いてはいけません。`agentfdr board --json` は同じボードを1回だけ出力します(スクリプト用)。
...
| ↳ 収束ヒント | 検知したループには、ツール*結果*から読み取ったヒントが付きます: 結果が毎回変化(収束の可能性)か、毎回同じ結果(停滞の可能性)か。あくまでヒントで「安全宣言」ではありません — どちらでもループ自体はフラグされ、判定が拮抗した場合は「停滞」側に倒します |
...
| `intent-drift` | 編集対象がプロンプトの依頼から離れ、次のプロンプトまで戻ってこなかった。判定するのは「プロンプトが名指ししたものを実際に開いた区間」だけ — 対象が明示されていなければ基準となる意図がないので、何も言いません |
...
- `thresholds` — 各検知器の閾値を上書き(`loopRepeats`、`loopMinCalls`、`loopRetryRepeats`、`errorStreak`、`contextBloatChars`、`tokenSpikeTokens`、`tokenSpikeRatio`、`cacheThrashTurns`、`fileChurnEdits`、`driftAnchorTurns`、`driftEditTurns`)。`loopRetryRepeats`(既定6)は「編集↔検証の交互反復」や「テスト/ビルド/lintの慣用句の反復」など、通常の作業に見えるループ形状に適用される閾値で、それ以外は `loopRepeats`(既定3)を使います。`driftEditTurns`(既定3)は intent-drift が必要とする「依頼外のファイルを編集し続けたターン数」で、横断的な変更が多いプロジェクトでは大きくしてください
...
サブエージェントは専用のトランスクリプト(`<session-id>/subagents/**/agent-*.jsonl` と、起動元のツール呼び出しIDを持つ `.meta.json`)に記録されるため、サブエージェントタブを開いたときと `blame` 実行時にだけ遅延パースします。古い Claude Code はサイドチェーンのターンをメインファイルに直接書いていましたが、どちらの形式でも同じツリーになります。
...
ボードはさらに3つのローカルソース(いずれも Claude のみ)を使います: 稼働プロセスのレジストリ `~/.claude/sessions/<pid>.json`(状態と tmux ペイン。Claude Code 自身が書き、プロセスが消えたエントリは無視)、現在のタスクを始めたプロンプトの全文を得るための `~/.claude/history.jsonl`、そして Claude デスクトップアプリ自身のセッション索引(macOS では `~/Library/Application Support/Claude/claude-code-sessions/<account>/<org>/local_*.json`。`AGENTFDR_DESKTOP_DIR` で変更可)— Desktop セッションのタイトル・休止中セッション・アカウントの判別に使います。指示は登録されたペインに対する `tmux send-keys` / `paste-buffer` の呼び出しで、私的なプロトコルは使わず、Claude Code に何も注入しません。
...
- [x] ループへの収束アノテーション(あくまでヒント表示、「安全宣言」にはしない)— 誤検知率が十分下がってから
- [x] 意図ドリフト検知 — ツール/ファイルの操作範囲がプロンプトの依頼内容から逸れ始めたターンをフラグ
- [x] サブエージェント/サイドチェーンのツリー表示
- [x] ライブセッションボード(`agentfdr board`)— 稼働中の全セッションを1枚のカンバンに、tmux セッションへの指示付き
...
コントリビューション歓迎です。特に新しい検知ヒューリスティックやビューアの改善。[CONTRIBUTING.md](CONTRIBUTING.md) をどうぞ。
