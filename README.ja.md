...
- **2アカウントを1画面で** — セッションに **CLI** / **Desktop**(Claude デスクトップアプリ内の Claude Code)のバッジが付き、フィルタでどちらかに絞れます。
...
ボードはさらに2つのローカルソース(どちらも Claude のみ)を使います: 稼働プロセスのレジストリ `~/.claude/sessions/<pid>.json`(状態と tmux ペイン。Claude Code 自身が書き、プロセスが消えたエントリは無視)と、現在のタスクを始めたプロンプトの全文を得るための `~/.claude/history.jsonl`。指示は登録されたペインに対する `tmux send-keys` / `paste-buffer` の呼び出しで、私的なプロトコルは使わず、Claude Code に何も注入しません。
