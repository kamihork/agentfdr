<div align="center">
  ...
  Claude Code がループした。ゴールから逸れた。気づけば200万トークンが溶けていた。<br><code>agentfdr</code> はその「なぜ」を、後からターン単位で解剖できるツールです。</p>
...
セットアップはこれだけです。Claude Code はすべてのセッションの完全な記録(transcript)を `~/.claude/projects/` に書き出しています。`agentfdr` はそれを読み込んで、人間が調査できる形に変えます。
...
- 📊 **プラン使用量** — 5時間ウィンドウ/日別/週間の消費を全プロジェクト横断で集計。プラン種別を自動検出し、予算を設定すると消費率を警告バーで表示
...
画面右に**解剖パネル**が常駐します(広い画面では常時表示、狭い画面ではスライドイン)。ターンをクリックすると内容が表示され、タイムラインが見えたまま **←/→** でターンを次々に調べられます(**Esc** で選択解除)。パネルの左端をドラッグすると幅を変更でき、設定は記憶されます。
...
transcript のフォーマットは公開 API ではないため、パーサーは「壊れないこと」を最優先に書かれています。未知の行タイプはメタイベントとして残し、壊れた行は数えてスキップ。スキーマ変更の影響は1つのアダプタモジュールに閉じ込めています。
